 ...
 */
 ...
 function (nsCurrentRec, nsRecord, nsUrl, nsFormat, nsMessage, nsDialog, nsSearch) {
     ...
     function checkStatus(msg) {
         ...
         var b = setInterval(function (msg) {
             ...
             //    console.log("checkstatus",taskStatus)
                           ...
                           var isPendingProcessed = false

                           var lineCount = currentRec.getLineCount({sublistId:"sublist"});
                           jQuery('#sublistrow0 :first-child').text()
                           console.log("check",jQuery('#sublistrow0 :first-child').text())
                          
                           if(lineCount>0)
                           {
                            for(var i=0; i <lineCount; i++)
                            {
                                externalid = jQuery('#sublistrow'+i+' :nth-last-child(3)').text()+"_"+jQuery('#sublistrow'+i+' :nth-last-child(5)').text()
                                console.log("externalid",externalid)
 
                              totalRecordSync.push( externalid)
                            }
                            console.log("totalRecordSync",geteverHourProcessQueue(totalRecordSync))
 
                            var processsData = geteverHourProcessQueue(totalRecordSync)
 
                             console.log("cdssss", jQuery('#sublistrow'+0+' :nth-last-child(2)').text())
                             updateStatus_Link(processsData)
                         
                           }
                          
                         //     console.log("totalRecordSync",processsData[ix].values.custrecord_everprocessingstatus[0].text)

                         //     jQuery('#sublistrow'+ix+':last-child').text(processsData[ix].values.custrecord_everprocessingstatus[0].text)
                         //   
                         ...
                         console.log("customRecord.length",customRecord.length)
                 if(lineCount==0)
                {
     ...
     function geteverHourProcessQueue(filter)
     {
         ...
         var customrecord_everhourprocessingqueueSearchObj = nsSearch.create({
             ...
             [
                 ...
                 nsSearch.createColumn({name: "custrecord_everprocessingstatus", label: "Status"}),
                 nsSearch.createColumn({name: "custrecord_everhourprocessingqueue_error", label: "Last Error"}),
                 nsSearch.createColumn({name: "custrecord_everhourprocessingqueue_inid", label: "Netsuite InternalId"})
     ...
     function updateStatus_Link(processsData)
     {
        
         for(var ix=0 ; ix<processsData.length; ix++)
         {
           jQuery('#sublistrow'+ix+' :nth-last-child(2)').text(processsData[ix].values.custrecord_everprocessingstatus[0].text)
           console.log(ix)
           var sublistrecord = jQuery('#sublistrow'+ix+' :nth-last-child(3)').text()
           // if( jQuery('#sublistrow'+ix+' :nth-last-child(2)').text(processsData[ix].values.custrecord_everprocessingstatus[0].text) == "Done" || jQuery('#sublistrow'+ix+' :nth-last-child(2)').text(processsData[ix].values.custrecord_everprocessingstatus[0].text) == "Error")
           // {
           //     continue
           // }
           if(sublistrecord=="Client" && processsData[ix].values.custrecord_everprocessingstatus[0].text=="Done" && processsData[ix].values.custrecord_everhourprocessingqueue_rcd[0].text=="Client")
           {
               var Clientlink="https://tstdrv2594818.app.netsuite.com/app/common/entity/custjob.nl?id="+processsData[ix].values.custrecord_everhourprocessingqueue_inid
              // jQuery('#sublistrow'+ix+' :nth-last-child(1)').html("<a href ='\""+link+"\"> </a>")
                 jQuery('#sublistrow'+ix+' :nth-last-child(1)').html('<html><a href="'+Clientlink+'">View</a></html>')
           }
          else if(sublistrecord=="Project" && processsData[ix].values.custrecord_everprocessingstatus[0].text=="Done" && processsData[ix].values.custrecord_everhourprocessingqueue_rcd[0].text=="Project")
           {
               var Projectlink="https://tstdrv2594818.app.netsuite.com/app/accounting/project/project.nl?id="+processsData[ix].values.custrecord_everhourprocessingqueue_inid
              // jQuery('#sublistrow'+ix+' :nth-last-child(1)').html("<a href ='\""+link+"\"> </a>")
                 jQuery('#sublistrow'+ix+' :nth-last-child(1)').html('<html><a href="'+Projectlink+'">View</a></html>')
           }
         else  if(sublistrecord=="Team" && processsData[ix].values.custrecord_everprocessingstatus[0].text=="Done" && processsData[ix].values.custrecord_everhourprocessingqueue_rcd[0].text=="Team")
           {
               var Teamlink="https://tstdrv2594818.app.netsuite.com/app/common/entity/employee.nl?id="+processsData[ix].values.custrecord_everhourprocessingqueue_inid
              // jQuery('#sublistrow'+ix+' :nth-last-child(1)').html("<a href ='\""+link+"\"> </a>")
                 jQuery('#sublistrow'+ix+' :nth-last-child(1)').html('<html><a href="'+Teamlink+'">View</a></html>')
           }
         else  if(sublistrecord=="Task" && processsData[ix].values.custrecord_everprocessingstatus[0].text=="Done" && processsData[ix].values.custrecord_everhourprocessingqueue_rcd[0].text=="Task")
           {
               var Tasklink="https://tstdrv2594818.app.netsuite.com/app/accounting/project/projecttask.nl?id="+processsData[ix].values.custrecord_everhourprocessingqueue_inid
              // jQuery('#sublistrow'+ix+' :nth-last-child(1)').html("<a href ='\""+link+"\"> </a>")
                 jQuery('#sublistrow'+ix+' :nth-last-child(1)').html('<html><a href="'+Tasklink+'">View</a></html>')
           }
           else  if(sublistrecord=="Expense" && processsData[ix].values.custrecord_everprocessingstatus[0].text=="Done" && processsData[ix].values.custrecord_everhourprocessingqueue_rcd[0].text=="Expense")
           {
               var Tasklink="https://tstdrv2594818.app.netsuite.com/app/accounting/transactions/exprept.nl?id="+processsData[ix].values.custrecord_everhourprocessingqueue_inid
              // jQuery('#sublistrow'+ix+' :nth-last-child(1)').html("<a href ='\""+link+"\"> </a>")
                 jQuery('#sublistrow'+ix+' :nth-last-child(1)').html('<html><a href="'+Tasklink+'">View</a></html>')
           }
           else  if(sublistrecord=="Expense Category" && processsData[ix].values.custrecord_everprocessingstatus[0].text=="Done" && processsData[ix].values.custrecord_everhourprocessingqueue_rcd[0].text=="Expense Category")
           {
               var Tasklink="https://tstdrv2594818.app.netsuite.com/app/accounting/otherlists/expcategory.nl?id="+processsData[ix].values.custrecord_everhourprocessingqueue_inid
              // jQuery('#sublistrow'+ix+' :nth-last-child(1)').html("<a href ='\""+link+"\"> </a>")
                 jQuery('#sublistrow'+ix+' :nth-last-child(1)').html('<html><a href="'+Tasklink+'">View</a></html>')
           }
           

          // jQuery('#sublistrow'+1+' :last-child').text(processsData[1].values.custrecord_everprocessingstatus[0].text+"min")
         }
